...
export const UI = {
  ...
  /**
   * Updates the sidebar visibility for transformation mode.
   */
  updateSidebarVisibility() {
    // Always hide scenario section and show exercise section for transformation mode
    this.elements.scenarioSection?.classList.add("hidden");
  ...
  updateStatus(type, message) {
    ...
    const { cls, dot } = STATUS_CONFIGS[type] || STATUS_CONFIGS.default;
    statusBox.className = `status-box p-3 rounded-xl border text-xs font-medium transition-all duration-300 flex items-center gap-2 ${cls}`;
    statusBox.innerHTML = `<span class="h-2 w-2 rounded-full ${dot}"></span><span>${message}</span>`;
  },
  ...
  appendMessage(text, sender, options = {}) {
    ...
    wrapper.className = `flex items-start mb-6 gap-3 max-w-[92%] md:max-w-[85%] ${sender === "user" ? "flex-row-reverse ml-auto" : "flex-row mr-auto"}`;
  ...
  showTypingIndicator(roleName) {
    ...
    wrapper.innerHTML = '<div class="w-8 h-8 rounded-full bg-gray-300 flex-shrink-0 mt-1"></div><div class="flex flex-col"><div class="text-xs text-gray-500 mb-1">' + roleName + ' schreibt...</div><div class="bg-white border border-slate-100 p-4 rounded-2xl rounded-tl-none shadow-sm flex gap-1"><span class="w-1.5 h-1.5 bg-slate-400 rounded-full animate-bounce"></span><span class="w-1.5 h-1.5 bg-slate-400 rounded-full animate-bounce [animation-delay:0.2s]"></span><span class="w-1.5 h-1.5 bg-slate-400 rounded-full animate-bounce [animation-delay:0.4s]"></span></div></div>';
  ...
  _createMessageBody(text, sender, { messageType = "default", roleName = "Partner", isIchMode = false }) {
    ...
    container.className = sender === "user" ? "flex flex-col items-end w-full" : "flex flex-col items-start w-full";
  ...
  setModeBadge(mode) {
    ...
    if (!modeBadge) return;
    const config = MODE_BADGE_CONFIG.transformation;
  ...
  setBriefingLoading(isLoading) {
    if (!isLoading) return;
    this.elements.briefingContent.innerHTML = '<div class="flex items-center text-gray-500"><svg class="animate-spin h-5 w-5 mr-3 text-blue-600" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" fill="none"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg><span>Lade Übung...</span></div>';
  ...
  toggleMobileMenu(forceClose = false) {
    ...
    sidebarOverlay.classList[action]("hidden");
    document.body.style.overflow = forceClose || isOpen ? "" : "hidden";
  },
  ...
  showFeedbackModal(feedback) {
    ...
    feedbackText && Utils.renderBoldMarkdownWithLineBreaks(feedbackText, feedback);
