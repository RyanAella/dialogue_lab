...
export function downloadCurrentTranscript() {
    ...
    const modeLabel = EXPORT_MESSAGES.modeLabels.transformation.display;
    ...
    const modePrefix = EXPORT_MESSAGES.modeLabels.transformation.filePrefix;
    let filenameParts = [modePrefix];
    const activeSelect = UI.elements.exerciseSelect;

    if (activeSelect && activeSelect.selectedIndex > 0) {
        const exerciseTitle = Utils.slugify(activeSelect.options[activeSelect.selectedIndex].text);
        filenameParts.push(exerciseTitle);
    }
