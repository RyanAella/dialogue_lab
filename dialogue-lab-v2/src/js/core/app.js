...
/**
 * Hides dropdowns that are not allowed in the current variant.
 */
function hideDisabledDropdowns() {
  const allowedModes = CURRENT_VARIANT.modes || [];

  // Hide scenario dropdown if SIMULATION not allowed
  const scenarioDropdown = document.getElementById('scenario-section');
  if (scenarioDropdown && !allowedModes.includes('simulation')) {
    scenarioDropdown.style.display = 'none';
  }

  // Hide exercise dropdown if TRANSFORMATION not allowed
  const exerciseDropdown = document.getElementById('exercise-section');
  if (exerciseDropdown && !allowedModes.includes('transformation')) {
